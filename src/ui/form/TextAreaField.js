...
/**
 * Create an UI textarea field element
 * @class
 * @alias module:/fw/ui/form/TextAreaField
 * @augments fw/ui/form/EditableFormElement
 * @param {Object} [config] - the configuration object of the textarea field
 ...
 * @param {number} [config/number] - fix the maximum length of the textarea
 ...
 */
class FwTextAreaField extends EditableFormElement {
    /**
    ...
    /**
     * Create the TextAreaField nodes
     * @method buildUI
     * @private
     */
    buildUI() {
        this.node.innerHTML = '<textarea spellcheck="false"></textarea>';
        this.textareaNode = this.node.getElementsByTagName('textarea')[0];
        this.setFocusableNode(this.textareaNode);
        this.setMaxLength(this.maxLength);
    }
    /**
     * Bind all events to the TextAreaField node
     * @method bindUI
     * @private
     */
    bindUI() {
     ...
     */
    onChange() {
        var value = this.textareaNode.value.trim();
    ...
    setMaxLength(maxLength) {
        ...
        this.textareaNode.setAttribute('maxlength', this.maxLength || '');
    }
    /**
     * Set the TextAreaField required
     * @method setRequired
    ...
    /**
     * Backup the TextAreaField value
     * @method backupValue
     * @private
     */
    backupValue() {
        this.originalValue = this.textareaNode.value;
    }
    /**
     * Restore the TextAreaField original value
     * @method restoreValue
     * @private
     */
    restoreValue() {
        this.textareaNode.value = this.originalValue;
    }
    /**
    ...
    setValue(value) {
        ...
        if (fw.isString(value) && value.length > 0) {
            this.value = this.textareaNode.value = value;
        } else {
            this.value = null;
...
module.exports = FwTextAreaField;
